...
** Last update Sat Jan 21 13:56:19 2017 Léandre Blanchard
*/
...
void			clear_pixel_buffer(t_pos pos, t_win *window)
{
  sfVector2i		xy;
      ...
      while (xy.x != window->width)
	{
	  if (lights == 1)
	    ...
	    my_put_pixel(window, xy.x, xy.y,
			 color_rgb((int)pos.pos.x % 5 * 10, (int)pos.pos.y % 5 * 15, 160));
	  xy.x = xy.x + 1;
	}
      xy.y = xy.y + 1;
    }

...
** Last update Thu Jan 19 17:26:33 2017 Léandre Blanchard
*/

#include "include/wolf.h"

float		distance(sfVector2f bloc, t_pos pos)
{
  float		dist;
  sfVector2f	my;
...
void		all_blocs(t_win *window, t_map *map, t_pos pos, t_rot rot)
{
  ...
  while (y != map->lines)
      ...
      while (i != map->cols)
	{
	  ...
	  rot.angle += M_PI / 2;
	  if (map->map[y][i] != '0')
	    put_square(window, rot, 5 * distance(xy_vector(i, y), pos),
...
void		display_walls(t_map *map, t_win *window, t_pos pos)
{
  ...
  rot.angle = M_PI / 180.0 * fmodf(pos.direction, 360) + M_PI;

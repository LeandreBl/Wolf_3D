...
** Last update Fri Jan 13 20:34:36 2017 Léandre Blanchard
*/
...
t_pos		events(t_map *map, t_win *window, t_pos pos)
{
  ...
  pos = move_events(map, window, pos);
...
void		gui_event_handler(t_map *map, t_win *window, t_pos pos)
{
  if (sfKeyboard_isKeyPressed(sfKeyF1) && option == 0)
    {
      ...
      walls_gui(map, window, 0, slot_nb(0));
    }
  if (sfKeyboard_isKeyPressed(sfKeyA) && walls_gui(map, window, 1, 1) == 1)
    {
      while (sfKeyboard_isKeyPressed(sfKeyA));
      add_door(map, window, pos);
    }
  if (sfKeyboard_isKeyPressed(sfKeyE) && walls_gui(map, window, 1, 1) == 1
      && option == 0)
  ...
  if (sfKeyboard_isKeyPressed(sfKeyF) && walls_gui(map, window, 1, 1) == 1
      && option == 0)
    destroy_bloc(map, window, pos);
  if (sfKeyboard_isKeyPressed(sfKeyG) && walls_gui(map, window, 1, 1) == 1
      && option == 0)
    create_bloc(map, window, pos);
}

void		destroy_bloc(t_map *map, t_win *window, t_pos pos)
{
  sfVector2f	ray;
...
void		create_bloc(t_map *map, t_win *window, t_pos pos)
{
  sfVector2f	ray;
  sfVector2f	prev;
